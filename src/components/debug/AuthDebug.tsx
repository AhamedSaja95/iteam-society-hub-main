...
export const AuthDebug: React.FC = () => {
  ...
  const checkPermissions = async () => {
    try {
      ...
      const registrationsTest = await supabase.from('event_registrations').select('id').limit(1);
      const membershipsTest = await supabase.from('memberships').select('id').limit(1);
          ...
          {debugInfo ? (
            <>
              <div>
                ...
                <span className={debugInfo.authStatus === 'Authenticated' ? 'text-green-600' : 'text-red-600'}>
              ...
              <div>
                <strong>Profile Role:</strong> {debugInfo.profile?.role || 'None'}
              </div>
              ...
              <div className="border-t pt-2">
                ...
                <div className="ml-2 space-y-1">
                  <div>Profiles: <span className={debugInfo.permissions.canReadProfiles ? 'text-green-600' : 'text-red-600'}>
                    {debugInfo.permissions.canReadProfiles ? '✓' : '✗'}
                  </span></div>
                  <div>Events: <span className={debugInfo.permissions.canReadEvents ? 'text-green-600' : 'text-red-600'}>
                    {debugInfo.permissions.canReadEvents ? '✓' : '✗'}
                  </span></div>
                  <div>Registrations: <span className={debugInfo.permissions.canReadRegistrations ? 'text-green-600' : 'text-red-600'}>
                    {debugInfo.permissions.canReadRegistrations ? '✓' : '✗'}
                  </span></div>
                  <div>Memberships: <span className={debugInfo.permissions.canReadMemberships ? 'text-green-600' : 'text-red-600'}>
