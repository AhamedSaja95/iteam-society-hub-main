...
export class RoleBasedDataService {
   ...
   */
  static async fetchDashboardData(userId: string, role: string): Promise<DashboardData> {
    console.log(`🔍 Fetching dashboard data for user ${userId} with role ${role}`);
  ...
  private static async fetchAdminData() {
    ...
    return {
      ...
      adminStats: this.calculateAdminStats(allUsers, allMemberships, allRegistrations)
    };
  }
  ...
  private static async fetchStaffData(userId: string) {
    ...
    const { data: managedRegistrations, error: managedRegistrationsError } = await supabase
      .from('event_registrations')
      .select(`
        *,
        profiles(first_name, last_name),
   ...
   */
  private static calculateBaseStats(profile: any, membership: any, events: any[], registrations: any[], notifications: any[]) {
    const now = new Date();
    const eventsRegistered = registrations.length;
    const eventsAttended = registrations.filter(r => r.attended).length;
    const attendanceRate = eventsRegistered > 0 ? Math.round((eventsAttended / eventsRegistered) * 100) : 0;
    const upcomingEvents = registrations.filter(r => r.events && new Date(r.events.event_date) > now).length;
    const unreadNotifications = notifications.filter(n => !n.read && !n.is_read).length;
   ...
   */
  private static calculateAdminStats(users: any[], memberships: any[], registrations: any[]) {
   ...
   */
  private static calculateStaffStats(createdEvents: any[], managedRegistrations: any[]) {
    const eventsCreated = createdEvents?.length || 0;
    const totalRegistrations = managedRegistrations?.length || 0;
    const attendedRegistrations = managedRegistrations?.filter(r => r.attended).length || 0;
